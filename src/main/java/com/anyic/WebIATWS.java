...
public class WebIATWS extends WebSocketListener {
    ...
    public String text;
    public boolean flag=false;//是否识别完成

    public Object lock=new Object();
    WebIATWS(String PcmFile){
        file=PcmFile;
    }
    WebIATWS(){
...
//    }
    ...
    public String getWenben(WebIATWS pcm) throws Exception {
        String hostUrl = "https://iat-api.xfyun.cn/v2/iat";
        String appid = "4ceb57b4";
        String apiKey = "a7a57ea357a0c5c3725be797b4437c5b";
        String apiSecret = "YWQ4MDAwMzJlOWY5NWQ4OTlkY2ExN2I3";
        // 构建鉴权url
        String authUrl = getAuthUrl(hostUrl, apiKey, apiSecret);
        OkHttpClient client = new OkHttpClient.Builder().build();
        //将url中的 schema http://和https://分别替换为ws:// 和 wss://
        ...
        // System.out.println(client.newCall(request).execute());
        //System.out.println("url===>" + url);
        WebSocket webSocket = client.newWebSocket(request, pcm);
        synchronized (lock){
            try {
                //主线程等待lock释放锁
                lock.wait();
